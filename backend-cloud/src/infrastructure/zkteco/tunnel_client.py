...
async def activate_member(member_id: str, card_no: str, zkteco_user_id: str, 
    ...
    if not gym_tunnel_url:
        logger.warning("[TUNNEL] GYM_TUNNEL_URL no está configurado. Omitiendo activación y guardando en cola.")
        try:
            ...
            logger.error(f"[TUNNEL] Error al guardar en pending_commands: {err}")
        return False
    ...
    try:
        ...
        import traceback
        logger.error(f"[TUNNEL] Excepción al activar miembro {member_id}: {type(e).__name__}: {str(e)}")
...
async def deactivate_member(member_id: str, zkteco_user_id: str, 
    ...
    if not gym_tunnel_url:
        logger.warning("[TUNNEL] GYM_TUNNEL_URL no está configurado. Omitiendo desactivación y guardando en cola.")
        try:
            supabase_client.table("pending_commands").insert({
                "member_id": member_id,
                "action": "deactivate",
